...
SPREADSHEET_NAME = ""
WORKSHEET_NAME = "" # Ensure this matches your tab name
SPREADSHEET_ID = "" # Add your ID here
RANGE_NAME = "" # Add your range here
...
def send_emails(scout_data, creds, worksheet):
    ...
    # Get IP address of SMTP server for security/stability
    try:
        host = socket.gethostbyname("smtp.gmail.com")
    except Exception:
        # Fallback to the hostname if resolution fails (though less robust)
        host = "smtp.gmail.com"

    with smtplib.SMTP_SSL(host, 465) as server:
        server.login(MY_EMAIL, MY_PASSWORD)

        for full_name, data in scout_data.items():
            # Generate the HTML table from the sheet for this specific scout
            # Update cell C2 with the current scout's full name for context
            worksheet.update_acell('C2', full_name)
            formatted_table = get_cell_html(creds)

            # --- 1. DEFINE FORMATTING VALUES ---
            # These keys MUST match the {placeholders} in your Spreadsheet cells
            format_values = {
                "scout_name": data["first_name"], # Corrected key to use 'first_name'
                "email": data["email"], # Includes additional recipients
                "last_name": data["last_name"],
                "parent_name": data["parent_first"], # Corrected key to use 'parent_first'
                "payment_link": data["payment"] # Corrected key to use 'payment'
            }

            # --- 2. APPLY FORMATTING TO BOTH SUBJECT AND BODY ---
            # .format(**format_values) safely maps keys to {placeholders}
            try:
                final_subject = data["subject"].format(**format_values) # Corrected key to use 'subject'
                final_body = data["body"].format(**format_values) # Corrected key to use 'body'
            except KeyError as e:
                print(f"Error: Spreadsheet contains a placeholder {{e}} that doesn't exist.")
                continue
            ...
            print(f"Successfully sent formatted email to {full_name}")
